        ...
        loop {
            tokio::select! {
                res = listeners.accept() => {
                    ...
                    let options = ingress_options.live_load();
                    // AcceptedStream implements AsyncRead + AsyncWrite, so one call covers
                    // both transports -- no Either split needed.
...
mod tests {
    ...
    async fn bootstrap_test(
        ...
        // captured before `listeners` is moved into the ingress below
